...
public class MessageDispatcher implements Closeable, IMessageListener {
  ...
  public MessageDispatcher(Config config) {
    ...
    try {
      ...
      log.error(LogMessage.buildLogMessage(50001, LogMessage.getTxid(), "class load error"), e);
    }
  ...
  public void onMessage(final String channel, byte[] message) {
    ...
    try {
        ...
        case TYPE_REQUEST: // Transaction(request): synchronous operation
          ...
          if (outputMessageToLogger) {
            if (objectIds.contains(channel) ||
              ...
              log.info(LogMessage.buildLogMessage(10000, LogMessage.getTxid(), "MONITOR|{}|{}|{}|{}|{}|/{}/{}|{}",
      ...
      log.error(LogMessage.buildLogMessage(50002, LogMessage.getTxid(), "onMessage failed"), e);
    }
  }
  ...
  public void start() {
        ...
        do {
          ...
          try {
            ...
            if (response == null || !response.statusCode.equals(Response.OK)) {
              log.warn(LogMessage.buildLogMessage(10001, LogMessage.getTxid(), "Unsuccessful transaction to EventManager: {}", response.statusCode));
            }
          } catch (InterruptedException e) {
            log.warn(LogMessage.buildLogMessage(10002, LogMessage.getTxid(), "Unsuccessful transaction to EventManager due to some internal error"));
          } catch (Exception e) {
            log.warn(LogMessage.buildLogMessage(10003, LogMessage.getTxid(), "EventManager may be inactive"));
          }
    ...
    log.info(LogMessage.buildLogMessage(10005, LogMessage.getTxid(), "started"));
  }
  ...
  public void join() throws InterruptedException {
    ...
    log.info(LogMessage.buildLogMessage(10005, LogMessage.getTxid(), "joining"));
  ...
  public void stop() {
    close();
    log.info(LogMessage.buildLogMessage(10006, LogMessage.getTxid(), "stop"));
  }
  ...
  public void close() {
    ...
    log.info(LogMessage.buildLogMessage(10007, LogMessage.getTxid(), "terminated"));
  }
  ...
  protected void requestToEventManager(
    ...
    if (!eventManagerQueue.offer(request)) {
      log.warn(LogMessage.buildLogMessage(10008, LogMessage.getTxid(), "EventManager request queue is full"));
    }
  }
  ...
  public void onReconnected() {
    log.info(LogMessage.buildLogMessage(10009, LogMessage.getTxid(), "reconnected"));
  ...
  public void onDisconnected() {
    log.warn(LogMessage.buildLogMessage(10010, LogMessage.getTxid(), "disconnected"));
    if (mode.contains(MODE.RESEND_SUBSCRIBE_ON_RECONNECTED)) {
