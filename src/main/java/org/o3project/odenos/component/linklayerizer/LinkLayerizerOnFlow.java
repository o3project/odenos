...
public class LinkLayerizerOnFlow {
  ...
  public void flowAddedLowerNw(
    ...
    if (!(FlowStatus.ESTABLISHING.toString().equals(status) && enable)) {
      log.info(LogMessage.buildLogMessage(10074, LogMessage.getSavedTxid(), "invalid lower flow. [status: {}, enable: {}].", status, enable));
      return;
    }
  ...
  protected void addLayerizedLinkbyLowerFlow(
    ...
    if ((FlowStatus.NONE.toString().equals(status))
      ...
      log.warn(LogMessage.buildLogMessage(10075, LogMessage.getSavedTxid(), "invalid flow's status."));
        return;
    }

    if (!enable) {
      log.warn(LogMessage.buildLogMessage(10076, LogMessage.getSavedTxid(), "invalid flow's disable."));
      return;
    }

    String lowerNwId = getNetworkIdByType(LinkLayerizer.LOWER_NETWORK);
    if (lowerNwId == null) {
      log.warn(LogMessage.buildLogMessage(10077, LogMessage.getSavedTxid(), "lower_nw not exist."));
      return;
    }
    ...
    if (srcBoundary == null || dstBoundary == null) {
      log.warn(LogMessage.buildLogMessage(10074, LogMessage.getSavedTxid(), "invalid lowerFlow."));
      return;
    }
    ...
    String layerizedId =
        getNetworkIdByType(LinkLayerizer.LAYERIZED_NETWORK);
    if (layerizedId == null) {
      log.warn(LogMessage.buildLogMessage(10078, LogMessage.getSavedTxid(), "laerized_nw not exist."));
      return;
    }

    Response resp = networkInterfaces.get(layerizedId).putLink(link);
    if (resp.isError("PUT")) {
      log.warn(LogMessage.buildLogMessage(10079, LogMessage.getSavedTxid(), "failed PUT Link. response: {}", resp.getBodyValue()));
      return;
    }
  ...
  protected boolean setLayerizedLinkStatus(Link link, Flow flow) {
    ...
    if ((link == null) || (flow == null)) {
      log.warn(LogMessage.buildLogMessage(10080, LogMessage.getSavedTxid(), "parameter is null"));
      return false;
    }
    ...
    switch (flowStatus) {
      ...
      default:
        log.warn(LogMessage.buildLogMessage(10081, LogMessage.getSavedTxid(), "unknown status: {}", status));
        return false;
    }
  ...
  protected LinkLayerizerBoundary getBoundaryByMatches(
    ...
    if (StringUtils.isEmpty(lowerNwId)
      ...
      log.warn(LogMessage.buildLogMessage(10082, LogMessage.getSavedTxid(), "invalid parameter"));
      return null;
    }
  ...
  protected LinkLayerizerBoundary getBoundaryByActions(
    ...
    if (outNode == null || outPort == null) {
      log.warn(LogMessage.buildLogMessage(10083, LogMessage.getSavedTxid(), "invalid lowerFlow's actions."));
      return null;
    }
